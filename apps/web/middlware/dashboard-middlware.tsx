...
export default async function AppMiddleware(req: NextRequest, user: User) {
  ...
  if (!default_school_id && path.startsWith('/dashboard')) {
    return NextResponse.redirect(new URL('/onboarding/create-school', req.url));
  }
  if (path === '/dashboard') {
    return NextResponse.redirect(new URL(`/dashboard/${default_school_id}`, req.url));
  }
  /*
