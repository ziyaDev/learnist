'use client'
import { Text, Group, Modal, Radio, Stack, TextInput, Title, Grid, Flex, Textarea, Fieldset, Button, Select, MultiSelect, ColorPicker, DEFAULT_THEME, Input } from "@mantine/core";
...
import { createFormContext, zodResolver, useForm } from "@mantine/form";
import { z } from "zod";
import { createClient } from "@/supabase/lib/client";
import { notifications } from "@mantine/notifications";
import { useRouter } from "next/navigation";
const positions = ['Manager', 'Administrator', 'School owner', "Worker"] as const
const socials = ['Facebook', 'Instagram', 'Google', 'Youtube', 'Whatsapp', "Just a friend"] as const
type OnboardingSetupContextProps = {
   school_id?: number
   setSchoolId: (school_id: number) => void
}
...
export default function OnboardingSetup() {
   ...
   return <OnboardingSetupContext.Provider value={{
         ...
         <Fieldset legend="Basic information">
            <Flex gap={'sm'} w={"100%"} align={"end"}  >
               ...
               <TextInput key={form.key('name')}
                  {...form.getInputProps('name')} w={"100%"} label="Name" placeholder="What's the name of your school" />
            </Flex>
         </Fieldset>
         <Fieldset legend="Contact information">
            <Input w={"100%"} label="Phone number" key={form.key('phone_number')}
               {...form.getInputProps('phone_number')} component={IMaskInput} mask="+213 000 000 000" placeholder="Your phone" />
            <TextInput w={"100%"} key={form.key('email')}
               {...form.getInputProps('email')} label="Email address" placeholder="example@example.com" />
         </Fieldset>
         <Fieldset legend="More optional details">
            <TextInput w={"100%"} label="Full address" placeholder="Where your school is located" key={form.key('address')}
               {...form.getInputProps('address')} />
            <Textarea w={"100%"} label="Additional info" placeholder="Give your school a short description" key={form.key('additional_info')}
               {...form.getInputProps('additional_info')} />
         </Fieldset>
         <Button w={'100%'} loading={isMutating} mt="xs" disabled={!form.isValid()} type="submit">Continue</Button>
      </form>
   </>
}
